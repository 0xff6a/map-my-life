...
RSpec.describe Workout, :type => :model do
  ...
  it 'the intensity must be either bruce lee, high, medium, low' do
    workout = Workout.new(activity: 'Running', date: '10/12/2014', duration: 35, intensity: 'blah') 
    expect(workout).not_to be_valid
  ...
  it 'the metric must be either min/km or min/mile' do
    workout = Workout.new(activity: 'Running', date: '10/12/2014', duration: 35, intensity: 'high', pace: 4, metric: 'hi') 
    expect(workout).not_to be_valid
    expect(workout.errors.messages[:metric]).to include('metric must be either min/km or min/mile')
  end

  it 'if a workout has a pace it must have a metric' do
    workout = Workout.new(activity: 'Running', date: '10/12/2014', duration: 35, intensity: 'high', pace: 4) 
    expect(workout).not_to be_valid
    expect(workout.errors.messages[:metric]).to include('a paced workout must have a metric')
  end
